...
void i64rangeset_remove(i64rangeset *rangeset, int64_t v1, int64_t v2, int *status, char *err) {
    ...
    if ((v2<=rangeset->stack->data[0])||(v1>=rangeset->stack->data[rangeset->stack->size-1])) return;
    if ((v1<=rangeset->stack->data[0]) && (v2>=rangeset->stack->data[rangeset->stack->size-1])) {
...
void i64rangeset_intersect(i64rangeset *rangeset, int64_t a, int64_t b, int *status, char *err) {
    // Remove all values not within [a, b) from the rangeset.
    *status = 1;
    if (rangeset->stack->size == 0) return; // nothing to remove
    if ((b<=rangeset->stack->data[0]) || (a>=rangeset->stack->data[rangeset->stack->size-1])) {
        // no overlap
        rangeset->stack->size = 0;
        return;
    }
    if ((a<=rangeset->stack->data[0]) && (b>=rangeset->stack->data[rangeset->stack->size-1])) {
